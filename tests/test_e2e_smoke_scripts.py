...
def test_e2e_smoke_scripts_do_not_reference_temporal_backend() -> None:
    ps1 = read_source("scripts/e2e-smoke.ps1")
    sh = read_source("scripts/e2e-smoke.sh")

    assert "UseTemporal" not in ps1
    assert "USE_TEMPORAL" not in sh
    assert "temporal" not in ps1.lower()
    assert "temporal" not in sh.lower()
